...
'''
一个连接的唯一标识是[server ip, server port, client ip, client port].
...
【而一个端口可以认为就是一个应用程序啦】
【至于是谁在这个应用程序中产生的若干端口进行分发，就是应用程序呗，目前认为是都封装到对应的库中了。】
'''
class TCPClient:

    def __init__(self, serverName="localhost", serverPort=12002):
